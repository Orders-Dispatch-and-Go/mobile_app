...
void auth_t::login(const QString &email, const QString &password) {
    ...
    auto *reply = m_client.post<QJsonObject, QJsonObject>(m_url_sign_in, sign_in_obj);

    connect(reply, &reply_t::finished, this,
        [reply, this](const QVariant &data) {
            auto token_object = data.value<QJsonObject>();
            emit on_token_ready(token_object);
            reply->deleteLater();
        });
    
    connect(reply, &reply_t::reply_error, this,
        [reply, this](const QString &err) {
            emit auth_error(err);
            reply->deleteLater();
        });
}
...
void auth_t::on_token_ready(const QJsonObject &token_object) {
    ...
    connect(reply, &reply_t::finished, this,
        [token_object, reply, this](const QVariant &data) {
            ...
            auto dto = user_dto_t::from_json(user_object);

            emit on_user_ready(token_object, dto);
            reply->deleteLater();
        });
...
void auth_t::on_user_ready(const QJsonObject &token_object, const user_dto_t &dto) {
